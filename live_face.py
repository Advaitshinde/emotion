...
def show_webcam() :
    ...
    def modify_h5_model(file_path):
        with h5py.File(file_path, 'r+') as f:
            ...
            for layer in config['config']['layers']:
                # Remove dtype in Conv2D and BatchNormalization initializers
                ...
                # Remove initializer attributes from SeparableConv2D
                if layer['class_name'] == 'SeparableConv2D':
                    ...
                    layer['config'].pop('depthwise_initializer', None)
                    layer['config'].pop('pointwise_initializer', None)
            
            # Save the modified configuration back to the model file
    ...
    # Update the model file before loading it
    model_path = r'video.h5'
